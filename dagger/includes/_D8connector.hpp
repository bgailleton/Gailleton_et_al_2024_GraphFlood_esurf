//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
#ifndef _D8connector_HPP
#define _D8connector_HPP
...
template <class T> class _D8connector {
public:
  ...
  _D8connector(int nx, int ny, T dx, T dy, T xmin, T ymin) {
    ...
    for (int i = 0; i < this->nnodes; ++i) {
      ...
      if (both_same_forcing && ((this->boundaries.force_giving(o) &&
                                ...
                                (this->boundaries.force_receiving(o) &&
                                 this->boundaries.force_receiving(i))))
        linkvalid = false;
